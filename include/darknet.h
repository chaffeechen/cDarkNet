...
*/
typedef enum {
    CLASSIFICATION_DATA, CLASSIFICATION_TXT_DATA, DETECTION_DATA, CAPTCHA_DATA, REGION_DATA, IMAGE_DATA, COMPARE_DATA, WRITING_DATA, SWAG_DATA, TAG_DATA, OLD_CLASSIFICATION_DATA, STUDY_DATA, DET_DATA, SUPER_DATA, LETTERBOX_DATA, REGRESSION_DATA, SEGMENTATION_DATA, INSTANCE_DATA, ISEG_DATA, DETECTION_AUG_DATA, DETECTION_C4_DATA, IMAGE_C4_DATA
} data_type;
...
LIB_API detection *get_network_boxes_double_thresh(network *net, int w, int h, float thresh, float iou_thresh ,float hier, int *map, int relative, int *num, int letter);
...
LIB_API float validate_detector_map0(char *datacfg, char *cfgfile, char *weightfile, float thresh_calc_avg_iou, const float iou_thresh, const int map_points, int letter_box, network *existing_net);
/*
++ 20190704
mapType 
