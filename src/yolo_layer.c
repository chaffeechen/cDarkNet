...
layer make_yolo_layer2(int batch, int w, int h, int n, int total, int *mask , float* class_weights , int classes, int max_boxes)
{
    int i;
    layer l = { (LAYER_TYPE)0 };
    l.type = YOLO;
...
void delta_yolo_class_hyper(float *output, float *delta, int index, int class_id, int classes, int stride, float *avg_cat, float scale ,int focal_loss)
{
    int n;
    if (delta[index + stride*class_id]){
...
*/
void delta_yolo_class_hyper2(float *output, float *delta, int index, int class_id, float* class_weights,int classes, int stride, float *avg_cat, float scale ,int focal_loss)
{
    int n;
    if (delta[index + stride*class_id]){
    ...
    for (b = 0; b < l.batch; ++b) {
            ...
            for (i = 0; i < l.w; ++i) {
                for (n = 0; n < l.n; ++n) {
                    ...
                    if (best_iou > l.truth_thresh) {
                        ...
                        delta_yolo_class_hyper2(l.output, l.delta, class_index, class_id, l.class_weights , l.classes, l.w*l.h, 0, l.class_scale ,l.focal_loss);
                        ...
                        delta_yolo_box(truth, l.output, l.biases, l.mask[n], box_index, i, j, l.w, l.h, state.net.w, state.net.h, l.delta, l.coord_scale*(2-truth.w*truth.h), l.w*l.h);
                    }
                }
            }
        }
        for(t = 0; t < l.max_boxes; ++t){
            ...
            if(mask_n >= 0){
                ...
                float iou = delta_yolo_box(truth, l.output, l.biases, best_n, box_index, i, j, l.w, l.h, state.net.w, state.net.h, l.delta, l.coord_scale*(2-truth.w*truth.h), l.w*l.h);
                ...
                delta_yolo_class_hyper2(l.output, l.delta, class_index, class_id, l.class_weights ,l.classes, l.w*l.h, &avg_cat, l.class_scale ,l.focal_loss);
...
int get_yolo_detections_double_thresh(layer l, int w, int h, int netw, int neth, float thresh, float iou_thresh, int *map, int relative, detection *dets, int letter)
{
    //printf("\n l.batch = %d, l.w = %d, l.h = %d, l.n = %d \n", l.batch, l.w, l.h, l.n);
    int i,j,n;
    float *predictions = l.output;
