...
box_label *read_boxes_aug(char *filename, int *n)
{
    ...
    int count = 0;
    //aug_data scan
    while(fscanf(file, "%d %f %f %f %f %d", &id, &x, &y, &w, &h , &trust_type) == 6){//5->6
        boxes = (box_label*)realloc(boxes, (count + 1) * sizeof(box_label));
        boxes[count].id = id;
        boxes[count].x = x;
        boxes[count].y = y;
        ...
        boxes[count].top    = y - h/2;
        boxes[count].bottom = y + h/2;
        boxes[count].trust_type = trust_type;//++
        ++count;
    }
    fclose(file);
    *n = count;
...
with aug_data matrix z = trust_type
*/
void fill_truth_detection_aug(const char *path, int num_boxes, float *truth ,int classes, int flip, float dx, float dy, float sx, float sy,
    int net_w, int net_h)
{
    char labelpath[4096];
    replace_image_to_label(path, labelpath);
    ...
    int count = 0;
    int i;
    box_label *boxes = read_boxes_aug(labelpath, &count);
    float lowest_w = 1.F / net_w;
    float lowest_h = 1.F / net_h;
    randomize_boxes(boxes, count);
    correct_boxes(boxes, count, dx, dy, sx, sy, flip);
    if (count > num_boxes) count = num_boxes;
    float x, y, w, h, t;//t = trust_type
    int id;
    int sub = 0;
    ...
    for (i = 0; i < count; ++i) {
        ...
        if (t > 10 || t < 0) {
            printf("\n Wrong data aug: t = %f \n", t);
            sprintf(buff, "echo %s \"Wrong annotation: h = %f\" >> bad_label.list", labelpath, t);
            system(buff);
            t = 0;
            if (check_mistakes) getchar();
        }
...
matrix load_labels_paths_txt(char **paths, int n, int k)
{
    matrix y = make_matrix(n, k);
    int i;
    for(i = 0; i < n; ++i){
        fill_truth_txt(paths[i], k, y.vals[i]);
    }
    return y;
}
...
data load_data_aug_detection(int n, char **paths, int m, int w, int h, int c, int boxes, int classes, int use_flip, float jitter, float hue, float saturation, float exposure, float degP, float degV, int mini_batch, int track, int augment_speed)
{
    ...
    d.y = make_matrix(n, 6*boxes);//5->6
    //+aug_data 20190619
    // d.z = make_matrix(n,boxes);
    for(i = 0; i < n; ++i){
        const char *filename = random_paths[i];
        ...
        //different function
        fill_truth_detection_aug(filename, boxes, d.y.vals[i] , classes, flip, dx, dy, 1./sx, 1./sy, w, h);
...
data load_data_aug_detection(int n, char **paths, int m, int w, int h, int c, int boxes, int classes, int use_flip, float jitter, float hue, float saturation, float exposure, float degP, float degV ,int mini_batch, int track, int augment_speed)
{
    ...
    for (i = 0; i < n; ++i) {
        ...
        fill_truth_detection_aug(random_paths[i], boxes, d.y.vals[i] , classes, flip, dx, dy, 1. / sx, 1. / sy, w, h);
...
void *load_thread(void *ptr)
{
    ...
    /*
    + DETECTION_AUG_DATA to support data with low trust
    */
    if (a.type == OLD_CLASSIFICATION_DATA){
        *a.d = load_data_old(a.paths, a.n, a.m, a.labels, a.classes, a.w, a.h);
    } else if (a.type == CLASSIFICATION_DATA){
        *a.d = load_data_augment(a.paths, a.n, a.m, a.labels, a.classes, a.hierarchy, a.flip, a.min, a.max, a.size, a.angle, a.aspect, a.hue, a.saturation, a.exposure, 0);
    } else if (a.type == CLASSIFICATION_TXT_DATA){
        *a.d = load_data_augment(a.paths, a.n, a.m, a.labels, a.classes, a.hierarchy, a.flip, a.min, a.max, a.size, a.angle, a.aspect, a.hue, a.saturation, a.exposure, 1);
...
data load_data_augment(char **paths, int n, int m, char **labels, int k, tree *hierarchy, int use_flip, int min, int max, int size, float angle, float aspect, float hue, float saturation, float exposure, int Version)
{
    if(m) paths = get_random_paths(paths, n, m);
    data d = {0};
    d.shallow = 0;
    ...
    if (Version == 0 ) {
        ...
        d.y = load_labels_paths_txt(paths,n,k);
    } else {
        d.y = load_labels_paths(paths, n, labels, k, hierarchy);
    }
...
void get_next_batch_aug(data d, int n, int offset, float *X, float *y , float* z)
{
    ...
    for(j = 0; j < n; ++j){
        ...
        memcpy(X+j*d.X.cols, d.X.vals[index], d.X.cols*sizeof(float));
        memcpy(y+j*d.y.cols, d.y.vals[index], d.y.cols*sizeof(float));
        // memcpy(z+j*d.z.cols, d.z.vals[index], d.z.cols*sizeof(float));
    }
}
