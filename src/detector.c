...
void train_detector2(char *datacfg, char *cfgfile, char *weightfile, int *gpus, int ngpus, int clear, int dont_show, int calc_map, int mjpeg_port)
{
    list *options = read_data_cfg(datacfg);
    char *train_images = option_find_str(options, "train", "data/train.txt");
    char *valid_images = option_find_str(options, "valid", train_images);
    ...
    args.d = &buffer;
    /*
    DETECTION_AUG_DATA ATT!!
    */
    args.type = DETECTION_AUG_DATA;// DETECTION_DATA (train_detector) -> DETECTION_AUG_DATA (train_detector2)
    args.threads = 64;    // 16 or 64
            ...
            for (i = 0; i < nboxes; ++i) {//遍历所有检测出的结果
                ...
                for (class_id = 0; class_id < classes; ++class_id) {//对每个检测出的结果，遍历所有类别
                    ...
                    if (prob > 0) {//如果该类别可能性大于0则记录下来并进行处理
                        ...
                        int truth_index = -1;
                        float max_iou = 0;
                        for (j = 0; j < num_labels; ++j)//遍历所有标注的groundtruth
                        {
                            box t = { truth[j].x, truth[j].y, truth[j].w, truth[j].h };
...
float validate_detector_map0_2stage(
    ...
    int mapType )
{
    ...
    for (i = nthreads; i < m + nthreads; i += nthreads) {
        ...
        for (t = 0; t < nthreads && i + t - nthreads < m; ++t) {
            ...
            float hier_thresh = 0;
            detection *dets;
            dets = get_network_boxes(&netDet, 1, 1, thresh, hier_thresh, 0, 0, &nboxes, 0 );
    ...
    int mapType )
{
    ...
    int *tp_for_thresh_det = (int*)calloc(classes, sizeof(int));
    int *fp_for_thresh_det = (int*)calloc(classes, sizeof(int));
                    ...
                    if (prob > 0) {//如果该类别可能性大于0则记录下来并进行处理
                        ...
                        if (prob_obj > thresh_calc_avg_iou && class_id == 0) {//0.25 当objectness的概率也大于阈值，应该判断为正样本，如果truth_index = -1那么对应的groundtruth类别与之不符合，所以是false positive
                            ...
                            if (detected_truth_index > -1 && found == 0) {
                                detected_avg_iou += detected_max_iou;
                                ++detected_tp_for_thresh;
                                tp_for_thresh_det[det_truth_class_id] += 1;
                            }
                            else {
                                detected_fp_for_thresh++;
                            }
    ...
    free(tp_for_thresh_cls);
    free(fp_for_thresh_cls);
    free(tp_for_thresh_det);
    free(fp_for_thresh_det);
    free(indexes);
    free(top1_acc_cls);
    free(topk_acc_cls);
    free(truth_topk_cls_count);
...
float validate_detector_map0(char *datacfg, char *cfgfile, char *weightfile, float thresh_calc_avg_iou, const float iou_thresh, const int map_points, int letter_box, network *existing_net)
{
    int j;
    list *options = read_data_cfg(datacfg);
    char *valid_images = option_find_str(options, "valid", "data/train.txt");
...
float validate_detector_map02(char *datacfg, char *cfgfile, char *weightfile, float thresh_calc_avg_iou, const float iou_thresh, const int map_points, network *existing_net, int mapType)
{
    int j;
    list *options = read_data_cfg(datacfg);
    char *valid_images = option_find_str(options, "valid", "data/train.txt");
    ...
    for (i = nthreads; i < m + nthreads; i += nthreads) {
        ...
        for (t = 0; t < nthreads && i + t - nthreads < m; ++t) {
            ...
            float hier_thresh = 0;
            detection *dets;
            dets = get_network_boxes(&net, 1, 1, thresh, hier_thresh, 0, 0, &nboxes, 0 );
            //detection *dets = get_network_boxes(&net, val[t].w, val[t].h, thresh, hier_thresh, 0, 1, &nboxes, letter_box); // for letter_box=1
            if (nms) do_nms_sort(dets, nboxes, l.classes, nms);
...
float validate_detector_map02d(char *datacfg, char *cfgfile, char *weightfile, float thresh_calc_avg_iou, const float iou_thresh, const int map_points, network *existing_net, int mapType)
{
    int j;
    list *options = read_data_cfg(datacfg);
    char *valid_images = option_find_str(options, "valid", "data/train.txt");
    ...
    for (i = nthreads; i < m + nthreads; i += nthreads) {
        ...
        for (t = 0; t < nthreads && i + t - nthreads < m; ++t) {
            ...
            for (i=0; i < nboxes ; i ++ ) {
                free(dets[i].prob);
                dets[i].prob = (float*)calloc(1,sizeof(float));
                dets[i].prob[0] = dets[i].objectness;
            }
            //detection *dets = get_network_boxes(&net, val[t].w, val[t].h, thresh, hier_thresh, 0, 1, &nboxes, letter_box); // for letter_box=1
            ...
            for (i = 0; i < nboxes; ++i) {
                ...
                if (1) {
                    float prob = dets[i].prob[0];//++20190623 prob of a target contains a target
                    class_id = 0;
                    if (prob > 0) {
                        detections_count++;
                        detections = (box_prob*)realloc(detections, detections_count * sizeof(box_prob));
    ...
    for (rank = 0; rank < detections_count; ++rank) {
        ...
        if (rank > 0) {
            int class_id;
            for (class_id = 0; class_id < 1; ++class_id) {
                pr[class_id][rank].tp = pr[class_id][rank - 1].tp;
                pr[class_id][rank].fp = pr[class_id][rank - 1].fp;
            }
        }
    ...
    mean_average_precision = mean_average_precision / 1;
    mean_average_f1 = mean_average_f1 / 1;
    printf("\n IoU threshold = %2.0f %%, ", iou_thresh * 100);
    if (map_points) printf("used %d Recall-points \n", map_points);
    else printf("used Area-Under-Curve for each unique Recall \n");
...
void test_detector_double_thresh(char *datacfg, char *cfgfile, char *weightfile, char *filename, float thresh, float iou_thresh ,
    float hier_thresh, int dont_show, int ext_output, int save_labels, char *outfile)
{
    list *options = read_data_cfg(datacfg);
    char *name_list = option_find_str(options, "names", "data/names.list");
    ...
    while (1) {
        ...
        int nboxes = 0;
        detection *dets = get_network_boxes_double_thresh(&net, im.w, im.h, thresh, iou_thresh , hier_thresh, 0, 1, &nboxes, letterbox);
        if (nms) do_nms_sort(dets, nboxes, l.classes, nms);
        draw_detections_v4(im, dets, nboxes, thresh, iou_thresh , names, alphabet, l.classes, ext_output);
        save_image(im, "predictions");
        if (!dont_show) {
            show_image(im, "predictions");
        }
        ...
        if (save_labels)
        {
            ...
            for (i = 0; i < nboxes; ++i) {
                ...
                float prob = 0;
                for (j = 0; j < l.classes; ++j) {
                    if (dets[i].prob[j] > iou_thresh && dets[i].prob[j] > prob) {
                        prob = dets[i].prob[j];
                        class_id = j;
                    }
                }
...
void test_detector_double_thresh_list(char *datacfg, char *cfgfile, char *weightfile, char *filename, float thresh, float iou_thresh ,
    float hier_thresh, int dont_show, int ext_output, int save_labels, char *outfile)
{
    list *options = read_data_cfg(datacfg);
    char *name_list = option_find_str(options, "names", "data/names.list");
...
void run_detector(int argc, char **argv)
{
    ...
    if (0 == strcmp(argv[2], "test")) test_detector(datacfg, cfg, weights, filename, thresh, hier_thresh, dont_show, ext_output, save_labels, outfile);
    ...
    else if (0 == strcmp(argv[2], "map0")) validate_detector_map0(datacfg, cfg, weights, thresh, iou_thresh, 11, 0, NULL);
        ...
        if (weightsCls) 
            if (strlen(weightsCls) > 0)
                if (weightsCls[strlen(weightsCls) - 1] == 0x0d) weightsCls[strlen(weightsCls) - 1] = 0;        
        validate_detector_map0_2stage(datacfg, cfg, weights, cfgCls , weightsCls ,thresh, iou_thresh, 1 ,0);
    }
    else if (0 == strcmp (argv[2],"map0_2stage_v2")) {//Top1 Classification
            //+20190710 2 stage map calculation
        assert(argc > 7);
        char *cfgCls = (argc > 6) ? argv[6]:0;
        char *weightsCls = (argc > 7) ? argv[7]:0;
        if (weightsCls) 
            if (strlen(weightsCls) > 0)
                if (weightsCls[strlen(weightsCls) - 1] == 0x0d) weightsCls[strlen(weightsCls) - 1] = 0;        
        validate_detector_map0_2stage(datacfg, cfg, weights, cfgCls , weightsCls ,thresh, iou_thresh, 2 ,0);
    }    
