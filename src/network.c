...
void fill_network_boxes_double_thresh(network *net, int w, int h, float thresh, float iou_thresh , float hier, int *map, int relative, detection *dets, int letter)
{
    int prev_classes = -1;
    int j;
    for (j = 0; j < net->n; ++j) {
...
detection *get_network_boxes_double_thresh(network *net, int w, int h, float thresh, float iou_thresh ,float hier, int *map, int relative, int *num, int letter)
{
    detection *dets = make_network_boxes(net, iou_thresh, num);
    fill_network_boxes_double_thresh(net, w, h, thresh, iou_thresh , hier, map, relative, dets, letter);
    return dets;
}
